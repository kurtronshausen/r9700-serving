...
another reason not to double up. (`vllm-gptq` and the older services do this
via `VLLM_PLE_CPU_OFFLOAD`; `vllm-gptq-aa` uses the newer image's
`--engram-config` default, which is also host RAM.) Pick one:
...
| `vllm-gptq-aa`        | Qwen3.8-Flash-Next-MXFP4-FP8-GPTQ, "AA2" checkpoint (same model repo as `vllm-gptq`) | 8006 | tcclaviger's **own TP4 recipe**, reproduced as faithfully as this repo's plumbing allows on his current image (`29.04.15` == `dev`), so a problem can be quoted back to him verbatim. `VLLM_PLE_CPU_OFFLOAD` is gone from this image — PLE placement is now the `--engram-config` default (`cpu_offload=true`), so the int6 PLE table is still pinned in host RAM via the UVA path, as with `vllm-gptq`. (The 29.04.9-era `cudaHostRegister` pinning that registers 0 bytes here is gone, which is why this newer image may boot where that one died — untested.) Differences from `vllm-gptq`: `--max-model-len 524288` (via a raised `max_position_embeddings`, the legitimate path), cudagraph size `[5]`, MTP 4 tokens, temp 0.6/top_k 20, no chat-template override; the crash-associated driver trio (`GPU_MAX_HW_QUEUES`/`HSA_ENABLE_*`) is ENABLED to match his config exactly — see the hang caveat in the `compose.yaml` service comment. Each deviation is annotated there. |
