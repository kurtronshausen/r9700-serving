...
# VLLM_REF=2a4cd640 (vllm-project/vllm PR #53896 head), mounted read-only into
# the runtime image (see compose.yaml).
...
class ChatCompletionRequest(OpenAIBaseModel):
    ...
    def check_tool_usage(cls, data):
        ...
        # Reject empty tools array, matching OpenAI API behavior. Erroneous
        # clients sometimes send `tools: []` with `tool_choice: "none"` (or
        # omitted); treat that as a no-tools request instead of erroring,
        # matching the effective default of tool_choice="none".
        if data.get("tools") == []:
            if data.get("tool_choice") in (None, "none"):
                data.pop("tools", None)
            else:
                raise VLLMValidationError(
